...
[View on Docker Hub 🐳](https://hub.docker.com/r/animeshry/decap-cms-standalone)
...
docker run -d \
  ...
  --name decap-cms \
  --restart=always \
  animeshry/decap-cms-standalone:latest
```
...
[See the example file stored in the repo here.](https://github.com/AnimeshRy/docker-decap-cms-standalone/blob/master/app/config.yml)

- Make sure to update `name`, `repo`, and `branch` to match your backend provider.
    i.e.:
    ```
        name: [github|gitlab]
        repo: animeshry/test-blog-content
        branch: main
    ```
